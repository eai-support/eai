...
- **DTE-016:** retain bounded, no-follow local publication and complete source digests. Bind each opened local source file to the inode that passed the initial bounded-file check, then revalidate the opened inode against the contained canonical path and unchanged ancestors before reading. Persist the local receipt through an opened confined regular-file descriptor rather than a path-based rename. Read caller-selected workflow evidence through the same bounded no-follow regular-file contract, including every parent identity plus a post-read descriptor/path/size check, before parsing or authenticated submission.
...
- **DTE-019, DTE-020, DTE-021, DTE-088:** consume exact template candidate bytes now, validate workflow inputs and collector parity, and keep the immutable released tag/commit as a deferred release gate without predicting a version. The candidate manifest must name only the canonical `.github/workflows/eai-app.yml` and `scripts/source-unknown-deployment-evidence.mjs` paths. The release gate must resolve the declared tag from the canonical template repository, prove that it names the exact reviewed candidate commit, and fetch both canonical remote files at that commit to verify their bytes against the pinned digests. Unified source revision proof includes canonical lowercase 40-hex `workflowBlobSha` and lowercase algorithm-qualified `collectorDigest` that the platform derives, verifies, and seals from the exact source commit.
- **DTE-023, DTE-025:** consume the template's immutable supply-chain pins and credential isolation.
- **DTE-031 through DTE-036, DTE-080:** bind dispatch, resume, retry, and terminal success to the exact operation, nonce, actor, tenant, source, workflow, commit, configuration, artifact, image, deployment, doctor, and target; make dispatch recovery safe after a lost response or crash; never select a latest operation. Validate every operation and tenant identifier as an opaque safe path segment before constructing either managed or legacy operation URLs. A terminal failed source status without accepted evidence requires a new operation and nonce. EAI-maintained completion requires a canonical merged SHA that exactly equals the unified revision commit. Success requires root `sourceStatus` in `handoff_pending` or `completed`, root and deployment `status: active`, `doctor.ready: true`, exact deployment/doctor identity, and the complete exact `sourceRevision` binding defined by the unified operation route.
- **DTE-091:** enforce the regional PublicAPI allowlist at the authenticated client boundary for all managed and legacy source-unknown calls, allowlist every browser-link, upload, and status origin before sending credentials, reject redirects across those authority boundaries, and load protected retry state before the first retry request. Every retry request uses the exact originally selected PublicAPI URL; missing local endpoint authority fails closed without network access.
- **DTE-036, DTE-061, DTE-080:** add portable, operation-bound doctor evidence while retaining the existing URL-only doctor command. Authenticated readiness requires the passing canonical `GET /api/eai/readiness` probe, not an unrelated authenticated smoke test. Evidence output must reject a link in every parent component and bind the opened output inode to the still-contained final path before writing owner-only bytes.
...
17. Legacy exact-operation requests reject empty, traversal, delimiter-bearing, or otherwise unsafe operation and target-tenant path segments before any network request.
18. Retry state without its exact originally selected regional PublicAPI URL is rejected instead of inheriting the caller's current endpoint.
19. Release verification fetches the canonical workflow and collector at the resolved producer tag commit and rejects either remote byte mismatch.
20. Configuration/source reads and receipt/doctor writes revalidate the opened inode against the contained canonical path after open, so an ancestor replacement cannot redirect bytes outside the approved root.
21. Unified source revisions require exact `workflowBlobSha` and `collectorDigest` fields; missing or malformed trusted-producer identity remains pending or fails closed.
22. Customer-owned retry loads protected state before any request, sends every read and mutation to that state's original regional PublicAPI URL, and requires a new source operation for terminal failure without accepted evidence.
23. EAI-maintained completion rejects a missing, malformed, or revision-mismatched merged SHA.
24. Workflow evidence ingestion rejects linked or replaced parents and a file changed or shortened during the bounded read.
