...
- **DTE-016:** retain bounded, no-follow local publication and complete source digests. Bind each opened local source file to the inode that passed the initial bounded-file check, then revalidate the opened inode against the contained canonical path and unchanged ancestors before reading. Persist the local receipt through an opened confined regular-file descriptor rather than a path-based rename. Read caller-selected workflow evidence through the same bounded no-follow regular-file contract before parsing or authenticated submission.
