...
- **DTE-016:** retain bounded, no-follow local publication and complete source digests. Read caller-selected workflow evidence through a bounded no-follow regular-file handle before parsing or authenticated submission.
...
- **DTE-086, DTE-087:** integrate current `main` semantically, keep each TypeScript implementation module under the repository's 300-line maintenance limit, and rerun repository-owned checks on the exact head.
