...
- **Goal:** Make it easy for the user to start the Legal Transactional Closing journey with complete, trusted context.
- **Required data:** Deal (deal record), Client (client entity), Counterparty (counterparty entity), Entity (legal entity), and TermSheet (deal term sheet).
...
- **Goal:** Turn the captured context into the next best action for Legal Transactional Closing without forcing the human reviewer to assemble the case manually.
- **Required data:** DiligenceRequest (diligence request), DiligenceResponse (diligence response), ChecklistItem (closing checklist item), DraftDocument (draft agreement), and MarkupRound (negotiation cycle).
...
- **Required data:** ApprovalMatrix (approval structure), SignaturePacket (signing packet), ClosingCondition (condition precedent), FundsFlow (funds-flow schedule), and ClosingSet (closing binder).
...
- **Required data:** ConsentRequest (required consent), OpinionLetter (opinion deliverable), FilingTask (post-close filing), PostCloseObligation (ongoing obligation), and MatterWorkspace (shared workspace).
...
EAI provides the safe service boundary for Legal Transactional Closing through Object Types, tenant-scoped resources, document processing, chat workflows, and CLI verification. For this scenario, the main records are `Deal`, `Client`, `Counterparty`, `Entity`, `TermSheet`, and 18 more Object Types.

| Process step | What EAI provides | Calling pattern |
| ------------ | ----------------- | --------------- |
| Step 1. Capture demand and context | Tenant-scoped intake resources for Deal (deal record), Client (client entity), Counterparty (counterparty entity), Entity (legal entity), and TermSheet (deal term sheet). Object Type validation, starter forms, optional document intake, and chat-guided capture keep the first record complete. | Define fields in `src/eai.config/object-types.ts`, run `eai types validate` and `eai types seed`, create initial `Deal` records with `useResources('Deal')` or `eai resources create Deal`, and keep browser calls behind `/api/eai/...`. |
| Step 2. Prepare the decision | Linked resource queries over DiligenceRequest (diligence request), DiligenceResponse (diligence response), ChecklistItem (closing checklist item), DraftDocument (draft agreement), and MarkupRound (negotiation cycle). Search, schema checks, document classification or RAG indexing, and chat summaries turn raw context into a prepared decision. | Use `useResources('Deal')` list/query/search patterns, verify shape with `eai resources schema`, use `useDocuments().upload/classify/ragIndex`, `eai docs upload`, `eai docs classify`, and `eai docs index` where supporting material exists, and send decision-support prompts through `useChat(workflowId, 'chat')` or `eai chat send`. |
| Step 3. Execute and collaborate | Resource updates and actions for ApprovalMatrix (approval structure), SignaturePacket (signing packet), ClosingCondition (condition precedent), FundsFlow (funds-flow schedule), and ClosingSet (closing binder). Status changes, assignments, notes, generated work packets, and chat support keep humans in control during execution. | Model actions in the Object Type code, call `client.resources.executeAction(type, id, action)` or the app hook equivalent, update records through the app service layer, and verify with `eai resources get/list/query`. |
| Step 4. Resolve, explain, and improve | Outcome resources for ConsentRequest (required consent), OpinionLetter (opinion deliverable), FilingTask (post-close filing), PostCloseObligation (ongoing obligation), and MatterWorkspace (shared workspace). Audit-friendly links, indexed final documents, reporting snapshots, and answer generation make the result explainable and reusable. | Persist outcomes as resources, index final material with `eai docs index` or `useDocuments().ragIndex`, send explanation prompts with `useChat` or `eai chat stream`, and use `eai resources aggregate/search` for reporting checks. |
