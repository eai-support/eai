...
EAI provides the safe service boundary for Construction Cost Estimation through Object Types, tenant-scoped resources, document processing, chat workflows, and CLI verification. For this scenario, the main records are `EstimatingOpportunity`, `Client`, `Project`, `DrawingSet`, `SpecificationSection`, and 17 more Object Types.

| Process step | What EAI provides | Calling pattern |
| ------------ | ----------------- | --------------- |
| Step 1. Capture demand and context | Tenant-scoped intake resources for EstimatingOpportunity (estimating opportunity), Client (client organisation), Project (project record), DrawingSet (drawing package), and SpecificationSection (spec section). Object Type validation, starter forms, optional document intake, and chat-guided capture keep the first record complete. | Define fields in `src/eai.config/object-types.ts`, run `eai types validate` and `eai types seed`, create initial `EstimatingOpportunity` records with `useResources('EstimatingOpportunity')` or `eai resources create EstimatingOpportunity`, and keep browser calls behind `/api/eai/...`. |
| Step 2. Prepare the decision | Linked resource queries over WorkBreakdownItem (work breakdown item), QuantityTakeoff (takeoff record), CostAssembly (assembled estimate element), LaborRate (labour rate), and MaterialRate (material price). Search, schema checks, document classification or RAG indexing, and chat summaries turn raw context into a prepared decision. | Use `useResources('EstimatingOpportunity')` list/query/search patterns, verify shape with `eai resources schema`, use `useDocuments().upload/classify/ragIndex`, `eai docs upload`, `eai docs classify`, and `eai docs index` where supporting material exists, and send decision-support prompts through `useChat(workflowId, 'chat')` or `eai chat send`. |
| Step 3. Execute and collaborate | Resource updates and actions for EquipmentRate (equipment rate), SubcontractBid (subcontract bid), VendorQuote (vendor quote), ContingencyRule (contingency rule), and AlternatesPackage (alternates package). Status changes, assignments, notes, generated work packets, and chat support keep humans in control during execution. | Model actions in the Object Type code, call `client.resources.executeAction(type, id, action)` or the app hook equivalent, update records through the app service layer, and verify with `eai resources get/list/query`. |
| Step 4. Resolve, explain, and improve | Outcome resources for EstimateVersion (versioned estimate), ReviewComment (review note), BidSubmission (submitted bid), AwardDecision (award decision), and ChangeOrderForecast (forecast change order). Audit-friendly links, indexed final documents, reporting snapshots, and answer generation make the result explainable and reusable. | Persist outcomes as resources, index final material with `eai docs index` or `useDocuments().ragIndex`, send explanation prompts with `useChat` or `eai chat stream`, and use `eai resources aggregate/search` for reporting checks. |
