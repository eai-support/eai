...
Insurance brokers juggle client onboarding, risk capture, market submissions, quote comparisons, binding, certificates, and renewal work across disconnected systems. A dedicated application gives the brokerage one operating model for revenue teams, service teams, and compliance reviewers.
...
- **Goal:** Make it easy for the user to start the Insurance Brokerage Operations journey with complete, trusted context.
- **Required data:** BrokerOrganization (top-level brokerage account), Producer (broker or account executive), ClientAccount (insured customer master), Prospect (pre-client pipeline record), and MarketCarrier (insurance carrier record).
...
- **Goal:** Turn the captured context into the next best action for Insurance Brokerage Operations without forcing the human reviewer to assemble the case manually.
- **Required data:** MarketAppetite (carrier appetite by segment), Submission (market submission package), ExposureSchedule (structured client exposure data), CoverageRequirement (requested coverage profile), and RiskAssessment (placement risk summary).
...
- **Required data:** QuoteRequest (request sent to a market), CarrierQuote (quote returned by a carrier), QuoteComparison (side-by-side market comparison), Binder (binding confirmation), and Policy (bound policy record).
...
- **Required data:** RenewalOpportunity (renewal workflow entry), CommissionSchedule (broker revenue terms), Invoice (billing record), Payment (received or remitted payment), and CertificateRequest (proof of insurance request).
...
EAI provides the safe service boundary for Insurance Brokerage Operations through Object Types, tenant-scoped resources, document processing, chat workflows, and CLI verification. For this scenario, the main records are `BrokerOrganization`, `Producer`, `ClientAccount`, `Prospect`, `MarketCarrier`, and 19 more Object Types.

| Process step | What EAI provides | Calling pattern |
| ------------ | ----------------- | --------------- |
| Step 1. Capture demand and context | Tenant-scoped intake resources for BrokerOrganization (top-level brokerage account), Producer (broker or account executive), ClientAccount (insured customer master), Prospect (pre-client pipeline record), and MarketCarrier (insurance carrier record). Object Type validation, starter forms, optional document intake, and chat-guided capture keep the first record complete. | Define fields in `src/eai.config/object-types.ts`, run `eai types validate` and `eai types seed`, create initial `BrokerOrganization` records with `useResources('BrokerOrganization')` or `eai resources create BrokerOrganization`, and keep browser calls behind `/api/eai/...`. |
| Step 2. Prepare the decision | Linked resource queries over MarketAppetite (carrier appetite by segment), Submission (market submission package), ExposureSchedule (structured client exposure data), CoverageRequirement (requested coverage profile), and RiskAssessment (placement risk summary). Search, schema checks, document classification or RAG indexing, and chat summaries turn raw context into a prepared decision. | Use `useResources('BrokerOrganization')` list/query/search patterns, verify shape with `eai resources schema`, use `useDocuments().upload/classify/ragIndex`, `eai docs upload`, `eai docs classify`, and `eai docs index` where supporting material exists, and send decision-support prompts through `useChat(workflowId, 'chat')` or `eai chat send`. |
| Step 3. Execute and collaborate | Resource updates and actions for QuoteRequest (request sent to a market), CarrierQuote (quote returned by a carrier), QuoteComparison (side-by-side market comparison), Binder (binding confirmation), and Policy (bound policy record). Status changes, assignments, notes, generated work packets, and chat support keep humans in control during execution. | Model actions in the Object Type code, call `client.resources.executeAction(type, id, action)` or the app hook equivalent, update records through the app service layer, and verify with `eai resources get/list/query`. |
| Step 4. Resolve, explain, and improve | Outcome resources for RenewalOpportunity (renewal workflow entry), CommissionSchedule (broker revenue terms), Invoice (billing record), Payment (received or remitted payment), and CertificateRequest (proof of insurance request). Audit-friendly links, indexed final documents, reporting snapshots, and answer generation make the result explainable and reusable. | Persist outcomes as resources, index final material with `eai docs index` or `useDocuments().ragIndex`, send explanation prompts with `useChat` or `eai chat stream`, and use `eai resources aggregate/search` for reporting checks. |
...
```text
Use the EAI App Template. Model Insurance Brokerage Operations with Object Types for BrokerOrganization, Producer, ClientAccount, Prospect, MarketCarrier. Use useResources for records and actions, useDocuments for uploads/classification/RAG where documents appear, useChat for workflow assistance, and verify with eai types/resources/docs/chat commands. Use eai publicapi only when no named command covers the required platform call.
```
