...
- **Goal:** Make it easy for the user to start the Claims Adjusting Operations journey with complete, trusted context.
- **Required data:** ClaimNotice (first notice of loss record), PolicySnapshot (coverage details at loss date), CoverageCheck (initial coverage validation), LossEvent (incident facts and timeline), and Claimant (party making the claim).
...
- **Goal:** Turn the captured context into the next best action for Claims Adjusting Operations without forcing the human reviewer to assemble the case manually.
- **Required data:** InsuredParty (covered customer entity), Adjuster (assigned handler), Assignment (claim ownership record), Reserve (financial reserve record), and Inspection (site or asset inspection).
...
- **Required data:** EvidenceItem (photo or document evidence), VendorEstimate (repair or service estimate), FraudSignal (potential fraud indicator), SettlementOption (settlement pathway), and PaymentApproval (approved settlement decision).
...
- **Required data:** RecoveryOpportunity (potential reimbursement path), LitigationHold (legal preservation control), CommunicationLog (stakeholder communications), SLAClock (service level timer), and AuditTrail (reviewable action log).
...
EAI provides the safe service boundary for Claims Adjusting Operations through Object Types, tenant-scoped resources, document processing, chat workflows, and CLI verification. For this scenario, the main records are `ClaimNotice`, `PolicySnapshot`, `CoverageCheck`, `LossEvent`, `Claimant`, and 18 more Object Types.

| Process step | What EAI provides | Calling pattern |
| ------------ | ----------------- | --------------- |
| Step 1. Capture demand and context | Tenant-scoped intake resources for ClaimNotice (first notice of loss record), PolicySnapshot (coverage details at loss date), CoverageCheck (initial coverage validation), LossEvent (incident facts and timeline), and Claimant (party making the claim). Object Type validation, starter forms, optional document intake, and chat-guided capture keep the first record complete. | Define fields in `src/eai.config/object-types.ts`, run `eai types validate` and `eai types seed`, create initial `ClaimNotice` records with `useResources('ClaimNotice')` or `eai resources create ClaimNotice`, and keep browser calls behind `/api/eai/...`. |
| Step 2. Prepare the decision | Linked resource queries over InsuredParty (covered customer entity), Adjuster (assigned handler), Assignment (claim ownership record), Reserve (financial reserve record), and Inspection (site or asset inspection). Search, schema checks, document classification or RAG indexing, and chat summaries turn raw context into a prepared decision. | Use `useResources('ClaimNotice')` list/query/search patterns, verify shape with `eai resources schema`, use `useDocuments().upload/classify/ragIndex`, `eai docs upload`, `eai docs classify`, and `eai docs index` where supporting material exists, and send decision-support prompts through `useChat(workflowId, 'chat')` or `eai chat send`. |
| Step 3. Execute and collaborate | Resource updates and actions for EvidenceItem (photo or document evidence), VendorEstimate (repair or service estimate), FraudSignal (potential fraud indicator), SettlementOption (settlement pathway), and PaymentApproval (approved settlement decision). Status changes, assignments, notes, generated work packets, and chat support keep humans in control during execution. | Model actions in the Object Type code, call `client.resources.executeAction(type, id, action)` or the app hook equivalent, update records through the app service layer, and verify with `eai resources get/list/query`. |
| Step 4. Resolve, explain, and improve | Outcome resources for RecoveryOpportunity (potential reimbursement path), LitigationHold (legal preservation control), CommunicationLog (stakeholder communications), SLAClock (service level timer), and AuditTrail (reviewable action log). Audit-friendly links, indexed final documents, reporting snapshots, and answer generation make the result explainable and reusable. | Persist outcomes as resources, index final material with `eai docs index` or `useDocuments().ragIndex`, send explanation prompts with `useChat` or `eai chat stream`, and use `eai resources aggregate/search` for reporting checks. |
...
```text
Use the EAI App Template. Model Claims Adjusting Operations with Object Types for ClaimNotice, PolicySnapshot, CoverageCheck, LossEvent, Claimant. Use useResources for records and actions, useDocuments for uploads/classification/RAG where documents appear, useChat for workflow assistance, and verify with eai types/resources/docs/chat commands. Use eai publicapi only when no named command covers the required platform call.
```
