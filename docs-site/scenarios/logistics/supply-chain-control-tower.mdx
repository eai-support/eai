...
- **Goal:** Make it easy for the user to start the Supply Chain Control Tower journey with complete, trusted context.
- **Required data:** Order (customer or replenishment order), Shipment (shipment record), InventoryNode (warehouse or node), SKU (stock-keeping unit), and DemandSignal (demand signal).
...
- **Goal:** Turn the captured context into the next best action for Supply Chain Control Tower without forcing the human reviewer to assemble the case manually.
- **Required data:** PurchaseOrder (upstream purchase order), ASN (advanced shipping notice), Carrier (transport carrier), TrackingMilestone (shipment milestone), and ExceptionAlert (exception alert).
...
- **Required data:** DelayReason (delay cause), ReallocationPlan (inventory reallocation plan), SupplierPromise (supplier commit date), WarehouseTask (warehouse action), and RoutePlan (route plan).
...
- **Required data:** ShortageCase (shortage case), CustomerCommitment (customer commitment), ETAUpdate (updated ETA), KPISnapshot (operational KPI snapshot), and RootCauseRecord (root-cause record).
...
EAI provides the safe service boundary for Supply Chain Control Tower through Object Types, tenant-scoped resources, document processing, chat workflows, and CLI verification. For this scenario, the main records are `Order`, `Shipment`, `InventoryNode`, `SKU`, `DemandSignal`, and 17 more Object Types.

| Process step | What EAI provides | Calling pattern |
| ------------ | ----------------- | --------------- |
| Step 1. Capture demand and context | Tenant-scoped intake resources for Order (customer or replenishment order), Shipment (shipment record), InventoryNode (warehouse or node), SKU (stock-keeping unit), and DemandSignal (demand signal). Object Type validation, starter forms, optional document intake, and chat-guided capture keep the first record complete. | Define fields in `src/eai.config/object-types.ts`, run `eai types validate` and `eai types seed`, create initial `Order` records with `useResources('Order')` or `eai resources create Order`, and keep browser calls behind `/api/eai/...`. |
| Step 2. Prepare the decision | Linked resource queries over PurchaseOrder (upstream purchase order), ASN (advanced shipping notice), Carrier (transport carrier), TrackingMilestone (shipment milestone), and ExceptionAlert (exception alert). Search, schema checks, document classification or RAG indexing, and chat summaries turn raw context into a prepared decision. | Use `useResources('Order')` list/query/search patterns, verify shape with `eai resources schema`, use `useDocuments().upload/classify/ragIndex`, `eai docs upload`, `eai docs classify`, and `eai docs index` where supporting material exists, and send decision-support prompts through `useChat(workflowId, 'chat')` or `eai chat send`. |
| Step 3. Execute and collaborate | Resource updates and actions for DelayReason (delay cause), ReallocationPlan (inventory reallocation plan), SupplierPromise (supplier commit date), WarehouseTask (warehouse action), and RoutePlan (route plan). Status changes, assignments, notes, generated work packets, and chat support keep humans in control during execution. | Model actions in the Object Type code, call `client.resources.executeAction(type, id, action)` or the app hook equivalent, update records through the app service layer, and verify with `eai resources get/list/query`. |
| Step 4. Resolve, explain, and improve | Outcome resources for ShortageCase (shortage case), CustomerCommitment (customer commitment), ETAUpdate (updated ETA), KPISnapshot (operational KPI snapshot), and RootCauseRecord (root-cause record). Audit-friendly links, indexed final documents, reporting snapshots, and answer generation make the result explainable and reusable. | Persist outcomes as resources, index final material with `eai docs index` or `useDocuments().ragIndex`, send explanation prompts with `useChat` or `eai chat stream`, and use `eai resources aggregate/search` for reporting checks. |
...
```text
Use the EAI App Template. Model Supply Chain Control Tower with Object Types for Order, Shipment, InventoryNode, SKU, DemandSignal. Use useResources for records and actions, useDocuments for uploads/classification/RAG where documents appear, useChat for workflow assistance, and verify with eai types/resources/docs/chat commands. Use eai publicapi only when no named command covers the required platform call.
```
