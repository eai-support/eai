...
export type CapabilitySetupMode =
  "portal_only" | "portal_setup_cli_consume" | "shared_setup" | "runtime_only";
...
const SENSITIVE_FIELD =
  /(?:api[-_]?key|secret|password|credential|connection[-_]?string|authorization|auth[-_]?header|private[-_]?key|signing[-_]?key|signature|cookie|sas|(?:access|refresh|oauth|bearer|id)[-_]?token|token$)/i;
const SAFE_REFERENCE_FIELD = /(?:ref|reference|name)$/i;
const SAFE_REFERENCE_VALUE =
  /^(?:kv|keyvault):\/\/[A-Za-z0-9._~:/-]+$|^https:\/\/[A-Za-z0-9.-]+\.vault\.azure\.net\/secrets\/[A-Za-z0-9-]+(?:\/[A-Za-z0-9-]+)?$/i;
const CREDENTIAL_TEXT =
  /(?:\bbearer\s+[^\s,;]+|\b(?:authorization|api[-_ ]?key|client[-_ ]?secret|password|connection[-_ ]?string|signing[-_ ]?key|signature|sas|sig)\b\s*[:=]\s*[^\s,;]+)/i;
...
function sanitizeDiagnosticText(value: string): string {
  return CREDENTIAL_TEXT.test(value)
    ? "Sensitive provider details were redacted."
    : value;
}
...
function readServerDetail(payload: unknown): {
  message?: string;
  code?: string;
} {
  ...
  return {
    ...
    setupMode:
      typeof value.setupMode === "string"
        ...
        : typeof value.setup_mode === "string"
          ? (value.setup_mode as CapabilitySetupMode)
          : undefined,
    portalRoute:
      typeof value.portalRoute === "string"
        ? value.portalRoute
        : typeof value.portal_route === "string"
          ? value.portal_route
          : undefined,
    cliOperations: Array.isArray(value.cliOperations)
      ? value.cliOperations.filter(
          (item): item is string => typeof item === "string",
        )
      : Array.isArray(value.cli_operations)
        ? value.cli_operations.filter(
            (item): item is string => typeof item === "string",
          )
        : undefined,
  };
...
function normalizeConnection(
  value: unknown,
): TenantCapabilityConnection | null {
  if (!isRecord(value)) return null;
...
export function assertNoSecretMaterial(value: unknown, path = "data"): void {
  ...
  for (const [key, item] of Object.entries(value)) {
    if (isSensitiveField(key) && !isSafeReference(key, item)) {
      throw new Error(
        `${path}.${key} is secret material. Configure credentials in Admin Portal and pass only a safe integration reference.`,
      );
    }
...
export function parseControlPlaneObject(
...
export class CapabilityControlPlaneClient {
  ...
  private async request(
    ...
    return readJson(
      await this.transport.requestPublicApi(path, options),
      operation,
    );
  }
  ...
  async listConnections(): Promise<TenantCapabilityConnection[]> {
    const payload = await this.request(
      this.tenantPath("/capability-connections"),
      "List capability connections",
    );
    return unwrapItems(payload)
      .map(normalizeConnection)
      .filter((item): item is TenantCapabilityConnection => item !== null);
  }

  async getConnection(key: string): Promise<TenantCapabilityConnection> {
    const payload = await this.request(
      this.tenantPath(
        `/capability-connections/${segment(key, "Capability key")}`,
      ),
      "Get capability connection",
    );
    const connection = normalizeConnection(
      isRecord(payload) && isRecord(payload.connection)
        ? payload.connection
        : payload,
    );
    if (!connection)
      throw new Error(
        `Capability connection ${key} returned an invalid response.`,
      );
    return connection;
  }
  ...
  async listAssets(kind: CapabilityAssetKind): Promise<unknown> {
    return this.request(
      this.tenantPath(this.assetCollection(kind)),
      `List ${kind} assets`,
    );
  }
  ...
  async getAsset(kind: CapabilityAssetKind, key: string): Promise<unknown> {
    return this.request(
      this.tenantPath(
        `${this.assetCollection(kind)}/${segment(key, `${kind} key`)}`,
      ),
      `Get ${kind} asset`,
    );
  }
  ...
  async updateAsset(
    ...
    if (version === null) {
      ...
      const currentRecord =
        isRecord(current) && isRecord(current.item) ? current.item : current;
      version =
        isRecord(currentRecord) && typeof currentRecord.version === "number"
          ? currentRecord.version
          : null;
    }
    if (version === null) {
      throw new Error(
        `${kind} ${key} did not include a version required for safe update.`,
      );
    }
    return this.request(
      this.tenantPath(
  ...
  async upsertAsset(
    ...
    try {
      ...
      if (!(error instanceof PlatformAPIRequestError) || error.status !== 404)
        throw error;
      return {
        action: "created",
  ...
  async listContent(domain: CapabilityContentDomain): Promise<unknown> {
    return this.request(
      this.tenantPath(`/content/${domain}`),
      `List ${domain}`,
    );
  }
  ...
  async listSharedAssetTypes(): Promise<unknown> {
    return this.request(
      this.tenantPath("/content/shared-asset-types"),
      "List shared asset types",
    );
  }

  async listSharedAssets(assetType: string): Promise<unknown> {
    return this.request(
      this.tenantPath("/content/shared-assets"),
      "List shared assets",
      {
  ...
  async getSharedAsset(assetType: string, assetKey: string): Promise<unknown> {
    return this.request(
      this.tenantPath(
        `/content/shared-assets/${segment(assetKey, "Shared asset key")}`,
      ),
      "Get shared asset",
      { params: { assetType: requiredValue(assetType, "Shared asset type") } },
    );
  ...
  async listBindings(appKey: string): Promise<unknown> {
    return this.request(
      this.bindingPath(appKey),
      "List app capability bindings",
    );
  }

  async setBinding(
    appKey: string,
    binding: AppCapabilityBindingRequest,
  ): Promise<unknown> {
  ...
  async setAssetBinding(
    appKey: string,
    binding: ContentCapabilityBindingRequest,
  ): Promise<unknown> {
  ...
  async removeBinding(appKey: string, bindingKey: string): Promise<unknown> {
    return this.request(
      `${this.bindingPath(appKey)}/${segment(bindingKey, "Binding key")}`,
      "Remove app capability binding",
      { method: "DELETE" },
    );
  }
  ...
  async validateBindings(
    ...
    return this.request(
      `${this.bindingPath(appKey)}/validate`,
      "Validate app capability bindings",
      {
        method: "POST",
  ...
  private bindingPath(appKey: string): string {
    return this.tenantPath(
      `/apps/${segment(appKey, "App key")}/capability-bindings`,
    );
  }
}
