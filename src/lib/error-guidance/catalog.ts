...
export const errorGuidanceCatalog = [
  ...
  },
  {
    ...
    evidenceToCheck: [
      ...
      'Whether the caller is using an app token or service identity rather than an end-user identity route.',
    ...
    fixes: [
      ...
      },
      {
        ...
        purpose: 'Replace root platform user lookup with the tenant-scoped V4 route in app-token or service-identity flows.',
        mutates: false,
      },
      {
        command: 'Use /v4/platform/tenants/<tenant-id>/users/<oid>/memberships',
        purpose: 'Replace root platform membership lookup with the tenant-scoped V4 route in app-token or service-identity flows.',
        mutates: false,
      },
      {
...
export function findGuidanceByCode(code: string): ErrorGuidance | undefined {
  const normalized = code.trim().toUpperCase();
  return errorGuidanceCatalog.find((entry) => entry.code.toUpperCase() === normalized);
}

export function findGuidanceByReason(reasonCode: string): ErrorGuidance | undefined {
...
export function findGuidanceByCodeOrReason(value: string): ErrorGuidance | undefined {
