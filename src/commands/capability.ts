...
function printConnection(connection: TenantCapabilityConnection): void {
  ...
  out.table([
    ...
    ['Configured', connection.configured ? chalk.green('yes') : chalk.yellow('no')],
    ['Bound', connection.bound ? chalk.green('yes') : chalk.yellow('no')],
    ['Runtime ready', connection.runtimeReady ? chalk.green('yes') : chalk.yellow('no')],
  ]);
}

function tenantPortalRoute(definition: CapabilityDefinition, tenantId: string): string | null {
...
function setupGuidance(definition: CapabilityDefinition, connection: TenantCapabilityConnection, tenantId: string) {
  ...
  switch (definition.setupMode) {
    ...
    case 'portal_setup_cli_consume':
      ...
      break;
    case 'shared_setup':
      nextAction = sharedCommands.length > 0
        ? `Use the typed CLI command family: ${sharedCommands.join(', ')}`
        : 'Use the typed CLI command family shown in capability documentation.';
      break;
    case 'runtime_only':
      nextAction = 'Bind the required runtime capability to the app, then validate app bindings.';
      break;
    default:
      nextAction = 'Inspect the capability documentation or ask a platform administrator for setup guidance.';
  }
...
export const capabilityCommand = new Command('capability')
  .description('Discover tenant capabilities and diagnose control-plane readiness');
...
capabilityCommand
  ...
  .action(async (options: CapabilityOptions) => {
    try {
      ...
      const byKey = new Map(connections.map((connection) => [connection.capabilityKey, connection]));
      const capabilities = definitions.map((definition) => ({
        ...definition,
        connection: byKey.get(definition.key) ?? null,
      }));
      if (normalizeFormat(options) === 'json') {
        out.json(sanitizeControlPlaneValue({ tenantId: ctx.tenantId, capabilities }));
        return;
      }
      ...
      for (const capability of capabilities) {
        ...
        const status = readiness?.runtimeReady ? chalk.green('ready') : chalk.yellow('not ready');
        out.info(`${chalk.cyan(capability.key)} — ${capability.setupMode ?? 'unknown'} — ${status}`);
      }
    } catch (error) {
      fail(error);
    }
...
capabilityCommand
  .command('status <key>')
  .description('Show entitlement, configuration, binding, and runtime readiness separately')
  .option('--tenant <id>', 'Tenant id (defaults to active tenant)')
  .option('--format <format>', 'Output format: text or json', 'text')
  .option('--json', 'Output raw JSON (deprecated, use --format json)', false)
  .action(async (key: string, options: CapabilityOptions) => {
    try {
      ...
      if (normalizeFormat(options) === 'json') {
        out.json(sanitizeControlPlaneValue({ tenantId: ctx.tenantId, connection }));
        return;
      }
...
capabilityCommand
  .command('setup <key>')
  .description('Show the governed Portal or typed CLI setup path without changing credentials')
  .option('--tenant <id>', 'Tenant id (defaults to active tenant)')
  .option('--format <format>', 'Output format: text or json', 'text')
  .option('--json', 'Output raw JSON (deprecated, use --format json)', false)
  .action(async (key: string, options: CapabilityOptions) => {
    try {
      ...
      if (!definition) throw new Error(`Capability definition ${key} was not found.`);
...
capabilityCommand
  .command('doctor [key]')
  .description('Diagnose the four independent readiness states for one or all capabilities')
  .option('--tenant <id>', 'Tenant id (defaults to active tenant)')
  .option('--format <format>', 'Output format: text or json', 'text')
  .option('--json', 'Output raw JSON (deprecated, use --format json)', false)
