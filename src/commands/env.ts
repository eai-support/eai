  ...
  .action(async (options) => {
    ...
    try {
      ...
      const envLines: string[] = [
        ...
        `# Source: ${APP_CONFIG_STORE} (label: ${label})`,
          ...
          try {
            ...
            if (options.includeSecrets) {
              ...
              try {
                ...
                envLines.push(`${kv.key}=${secretValue.trim()}`);
                out.success(`${kv.key} = ${chalk.dim('[from Key Vault]')}`);
              } catch {
                envLines.push(`# ${kv.key}= # Key Vault reference (access denied)`);
                out.warn(`${kv.key} — Key Vault access denied`);
              }
            } else {
              envLines.push(`# ${kv.key}= # Key Vault reference — use --include-secrets to resolve`);
              out.info(`${kv.key} = ${chalk.dim('[Key Vault ref — skipped]')}`);
            }
          } catch {
            envLines.push(`${kv.key}=${kv.value}`);
          }
        } else {
          envLines.push(`${kv.key}=${kv.value}`);
