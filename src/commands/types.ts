...
function getStorageBindingScope(
  binding: unknown,
  key: string,
...
export function validateObjectTypeStorageMetadata(
  ...
  if (
    !VALID_STORAGE_BACKENDS.includes(
      backend as (typeof VALID_STORAGE_BACKENDS)[number],
    )
  ) {
    issues.push(
      `storageBackend "${backend}" must be one of: ${VALID_STORAGE_BACKENDS.join(", ")}`,
    );
  }

  if (
    !VALID_STORAGE_METADATA_STATUSES.includes(
      storageMetadataStatus as (typeof VALID_STORAGE_METADATA_STATUSES)[number],
    )
  ) {
    issues.push(
      `storageMetadataStatus "${storageMetadataStatus}" must be one of: ${VALID_STORAGE_METADATA_STATUSES.join(", ")}`,
    );
  }
  ...
  if (backend === "postgresql") {
    ...
    if (missing.length > 0) {
      issues.push(
        formatStorageFieldIssue("PostgreSQL storageBinding", missing),
      );
    }
    ...
    if (missing.length > 0) {
      issues.push(
        formatStorageFieldIssue("DocumentDB storageBinding", missing),
      );
    }
...
function describeTenantResolutionSource(
...
function compareRemoteDocs(
  a: RemoteObjectTypeDocument,
  b: RemoteObjectTypeDocument,
): number {
...
function extractRemoteTypeState(payload: unknown): {
  ...
  if (isRecord(payload) && Array.isArray(payload.docs)) {
    dedupeRemoteObjectTypeDocs(extractRemoteObjectTypeDocs(payload)).forEach(
      (value) => {
...
function extractRemoteObjectTypeDocs(
  payload: unknown,
): RemoteObjectTypeDocument[] {
  if (!isRecord(payload) || !Array.isArray(payload.docs)) {
    return [];
  ...
  return payload.docs
    .filter(
      (value): value is RemoteObjectTypeDocument =>
    ...
    .map((value) => ({
      ...
      storageMetadataStatus:
        typeof value.storageMetadataStatus === "string"
...
export async function describeFailedPlatformResponse(
  ...
  const truncatedDetail =
    detail.length > 500 ? `${detail.slice(0, 497)}...` : detail;
...
async function archiveDuplicateRemoteTypes(
  ...
  for (const duplicate of duplicates) {
    ...
    if (!response.ok) {
      throw new Error(
        `archive failed: ${await describeFailedPlatformResponse(response)}`,
      );
    }
...
export function summarizeAppObjectTypePublish(
  ...
  return {
    ...
    updated,
    failed,
    verification: readTypeSeedVerification(
      tenantId,
...
function syncResultObjectTypeSlug(
...
export async function summarizeResourceApiSchemaSync(
  ...
  const requestedSlugs = new Set(
    requestedObjectTypes.map((objectType) => toObjectTypeSlug(objectType)),
  );
  ...
  const missingObjectTypes = [...requestedSlugs].filter(
    (slug) => !successfulResultSlugs.has(slug),
  );
  ...
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    if (typeof client.getPublishedObjectTypes === "function") {
      ...
      if (!publishedResponse.ok) {
        throw new Error(
          `published type re-fetch failed: ${publishedResponse.status} ${publishedResponse.statusText}`,
        );
      }
      ...
      if (
        ...
        for (const requestedType of requestedTypes) {
          ...
          if (!preciseResponse.ok) {
            throw new Error(
              `published type re-fetch failed for ${requestedType}: ${preciseResponse.status} ${preciseResponse.statusText}`,
            );
          }
          preciseDocs.push(
            ...extractRemoteObjectTypeDocs(
              (await preciseResponse.json()) as unknown,
            ),
          );
        }
    ...
    if (!schemaResponse.ok) {
      throw new Error(
        `schema re-fetch failed: ${schemaResponse.status} ${schemaResponse.statusText}`,
      );
    }
...
Examples:
  ...
  .action(async (options) => {
    ...
    const spinner =
      options.format === "json" ? null : ora("Loading Object Types...").start();
    ...
    if (
      ...
      exitWithError(
        ...
        {
          field:
            "--tenant-key when using --tenant-id with multiple tenant scopes",
        },
        options.format,
      );
    }
    ...
    const defaultValueIssues =
      collectTypeDefaultValueValidationIssues(objectTypes);
    ...
    const keysToSeed = await selectTenantKey(
      objectTypes,
      options.tenantKey,
    ...
    for (const tenantKey of keysToSeed) {
      ...
      try {
        ...
        if (appPublishOutcome.result) {
          ...
          if (options.format !== "json") {
            ...
            out.info(
              `Result: ${chalk.green(`${appPublishOutcome.result.created} created`)}, ${chalk.cyan(`${appPublishOutcome.result.updated} updated`)}, ${chalk.red(`${appPublishOutcome.result.failed} failed`)}`,
            );
            const verification = appPublishOutcome.result.verification;
            if (verification?.converged) {
              out.success(
                `Verification: converged (${verification.matchedTypes.length}/${verification.requestedTypes.length} published remotely)`,
              );
        ...
        if (options.format !== "json") {
          out.warn(
            `${appManifestFallbackReason ?? "App manifest publish unavailable"}; falling back to direct Object Type writes.`,
          );
        }
      } catch (err) {
      ...
      try {
        ...
        if (!remoteRes.ok) {
          throw new Error(
            `remote lookup failed: ${await describeFailedPlatformResponse(remoteRes)}`,
          );
        }
      ...
      for (const type of types) {
        ...
        try {
          ...
          if (duplicates.length > 0) {
            const archivedCount = await archiveDuplicateRemoteTypes(
              client,
              duplicates,
            );
            if (archivedCount > 0) {
              remoteDocs = remoteDocs.filter(
                (doc) =>
                  !duplicates.some((duplicate) => duplicate.id === doc.id),
              );
            }
          }
          ...
          if (existing?.id) {
            ...
            if (updateRes.ok) {
              const archivedSuffix =
                duplicates.length > 0
                  ? chalk.dim(
                      ` + archived ${duplicates.length} duplicate${duplicates.length === 1 ? "" : "s"}`,
                    )
                  : "";
              if (typeSpinner) {
                typeSpinner.succeed(
                  `  ${type.name} ${chalk.cyan("(updated)")}${archivedSuffix}`,
                );
              }
              updated++;
              ...
              if (typeSpinner) {
                typeSpinner.fail(
                  `  ${type.name} — update failed: ${await describeFailedPlatformResponse(updateRes)}`,
                );
              }
              failed++;
            }
            ...
            if (createRes.ok) {
              ...
              if (typeSpinner) {
                typeSpinner.succeed(
                  `  ${type.name} ${chalk.green("(created)")}${archivedSuffix}`,
                );
              }
              created++;
              remoteDocs.push({
              ...
              if (typeSpinner) {
                typeSpinner.fail(
                  `  ${type.name} — create failed: ${await describeFailedPlatformResponse(createRes)}`,
                );
              }
              failed++;
            }
          ...
          if (typeSpinner) {
            typeSpinner.fail(
              `  ${type.name} — ${err instanceof Error ? err.message : String(err)}`,
            );
          }
          failed++;
        }
      ...
      if (options.format !== "json") {
        out.info(
          `Result: ${chalk.green(`${created} created`)}, ${chalk.cyan(`${updated} updated`)}, ${chalk.red(`${failed} failed`)}`,
        );
        if (verification.converged) {
          out.success(
            `Verification: converged (${verification.matchedTypes.length}/${verification.requestedTypes.length} published remotely)`,
      ...
      for (const type of types) {
        ...
        for (const prop of type.properties) {
          ...
          if (
            ...
            warns.push(
              `property "${prop.name}" has options but type is "${prop.type}" (not select)`,
            );
          }
        }
        ...
        for (const link of type.linkTypes) {
          ...
          if (
            ...
            issues.push(
              `link "${link.name}" has invalid cardinality "${link.cardinality}"`,
            );
          }
        }
        ...
        for (const action of type.actions) {
          ...
          if (
            !["tenant-viewer", "tenant-builder", "tenant-admin"].includes(
              action.requiredRole,
            )
          ) {
            issues.push(
              `action "${action.name}" has invalid requiredRole "${action.requiredRole}"`,
            );
          }
          for (const effect of action.sideEffects) {
            if (
              !["set_field", "set_timestamp", "set_user"].includes(effect.type)
            ) {
              issues.push(
                `action "${action.name}" side effect has invalid type "${effect.type}"`,
              );
            }
            if (effect.type === "set_field" && !propNames.has(effect.field)) {
              warns.push(
                `action "${action.name}" side effect references unknown field "${effect.field}"`,
              );
            }
          }
        }
        ...
        if (issues.length === 0 && warns.length === 0) {
          out.success(
            `${type.name} — ${type.properties.length} props, ${type.linkTypes.length} links, ${type.actions.length} actions`,
          );
        } else {
          if (issues.length > 0) {
            out.error(`${type.name}`);
...
Examples:
  ...
  .action(async (options) => {
    ...
    const entries = keysToDiff.map(
      (key) =>
        [key, objectTypes[key] as ObjectTypeDefinition[] | undefined] as const,
    );
...
typesCommand
  ...
  .option(
    "--output <path>",
    "Output file path",
    "src/eai.config/object-types.generated.ts",
  )
  .addHelpText(
    "after",
    `
