...
describe("CapabilityControlPlaneClient", () => {
  test("normalizes definitions and preserves four independent readiness states", async () => {
    ...
    const client = new CapabilityControlPlaneClient(
      { requestPublicApi },
      "tenant-123",
    );
    ...
    expect(requestPublicApi).toHaveBeenNthCalledWith(
      2,
  ...
  test("uses typed CRUD envelopes and optimistic versions for AI assets", async () => {
    const requestPublicApi = vi
      .fn()
      .mockResolvedValueOnce(
        response({ key: "default-chat", version: 4, data: {} }),
      )
      .mockResolvedValueOnce(
        response({ key: "default-chat", version: 5, data: {} }),
      )
      .mockResolvedValueOnce(
        response({ key: "new-chat", version: 1, data: {} }),
      )
  ...
  test("stores logical app bindings without tenant record ids", async () => {
    const requestPublicApi = vi
      .fn()
      .mockResolvedValue(response({ bindings: [] }));
    const client = new CapabilityControlPlaneClient(
      { requestPublicApi },
      "tenant-123",
    );
  ...
  test("sends canonical capability requirements to persisted binding validation", async () => {
    const requestPublicApi = vi
      .fn()
      .mockResolvedValue(response({ valid: true, bindings: [] }));
    const client = new CapabilityControlPlaneClient(
      { requestPublicApi },
      "tenant-123",
    );
  ...
  test("rejects outbound secret material and redacts accidental inbound credentials", () => {
    expect(() =>
      assertNoSecretMaterial({ clientSecret: "do-not-send" }),
    ).toThrow(/Admin Portal/);
    expect(() =>
      assertNoSecretMaterial({ providerApiKey: "do-not-send" }),
    ).toThrow(/Admin Portal/);
    expect(() =>
      assertNoSecretMaterial({ authorization: "Bearer do-not-send" }),
    ).toThrow(/Admin Portal/);
    expect(() => assertNoSecretMaterial({ signingKey: "do-not-send" })).toThrow(
      /Admin Portal/,
    );
    expect(() => assertNoSecretMaterial({ sas: "sig=do-not-send" })).toThrow(
      /Admin Portal/,
    );
    expect(() =>
      assertNoSecretMaterial({ credentialRef: "raw-secret" }),
    ).toThrow(/Admin Portal/);
    expect(() =>
      assertNoSecretMaterial({
