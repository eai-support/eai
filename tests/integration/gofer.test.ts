...
describe('eai gofer refresh', () => {
  ...
  test('records the current managed snapshot on the first refresh without rewriting matching files', async () => {
    ...
    expectDisplayedMessage(result, 'Recorded the current state in `.eai-manifest.json`');
    await expectFileExists(ctx, '.eai-manifest.json');
    await expectFileContains(ctx, '.eai-manifest.json', '".github/copilot-instructions.md"');
  });
  ...
  test('detects local edits as conflicts and only overwrites them when forced', async () => {
    ...
    await expectFileContains(ctx, '.github/copilot-instructions.md', 'LOCAL CUSTOMIZATION');
    ...
    const backups = await listFilesRecursive(join(env.dir, '.specify', '_backup', 'gofer-refresh'));
    expect(backups.some((path) => path.endsWith(join('.github', 'copilot-instructions.md')))).toBe(true);
  });
});
