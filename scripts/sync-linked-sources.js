...
const GOFER_REPO = 'https://github.com/eai-tools/gofer.git';
...
export function extractEnterprisePackageVersions(lockfile) {
  ...
  return Object.fromEntries(packageEntries.map(([name, pkg]) => {
    return [
...
function buildTemplateMetadata(workdir) {
  ...
  return {
    ...
    packageLockSha256: crypto.createHash('sha256').update(packageLockText).digest('hex'),
...
function syncGoferResourcesFromCheckout(workdir, commit, describe) {
  ...
  for (const [sourceRelativeDir, targetRelativeDir] of GOFER_EXTRA_RESOURCE_MAPPINGS) {
...
function main() {
  const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'eai-cli-linked-sources-'));
  ...
  try {
    ...
    const manifestChanged = JSON.stringify(currentManifest) !== JSON.stringify(nextManifest);
