...
if [[ -f "$INDEX_HTML" ]]; then
  pass "registry/index.html exists"
else
  fail "registry/index.html missing: $INDEX_HTML"
fi

section "TC042: Version listing contains version number(s)"
if grep -q "$PKG_VERSION" "$INDEX_HTML" 2>/dev/null; then
  pass "index.html contains version $PKG_VERSION"
else
  fail "index.html missing version $PKG_VERSION"
fi

section "TC043: Version listing contains .npmrc instructions"
if grep -q '@eai-tools:registry=https://eai-tools.github.io/eai-cli/registry' "$INDEX_HTML" 2>/dev/null; then
  pass "index.html has .npmrc configuration"
else
  fail "index.html missing .npmrc configuration"
fi
if grep -q 'npm install -g @eai-tools/cli' "$INDEX_HTML" 2>/dev/null; then
  pass "index.html has npm install command"
else
  fail "index.html missing npm install command"
fi

section "TC044: Version listing has generated comment"
if grep -q 'Generated by scripts/generate-registry.cjs' "$INDEX_HTML" 2>/dev/null; then
  pass "index.html has generated comment"
else
  fail "index.html missing generated comment"
fi
...
NPX_LINES=$(grep -n 'npx @eai-tools/cli' "$DEPLOY_MDX" 2>/dev/null | cut -d: -f1)
NPX_FAIL=0
for line in $NPX_LINES; do
  # Search backwards from this line for .npmrc setup (within 15 lines)
  start=$((line - 15))
  if (( start < 1 )); then start=1; fi
  if sed -n "${start},${line}p" "$DEPLOY_MDX" | grep -q '@eai-tools:registry='; then
    pass "npx at line $line has preceding .npmrc setup"
  else
    fail "npx at line $line missing preceding .npmrc setup"
    NPX_FAIL=1
  fi
done
if [[ -z "$NPX_LINES" ]]; then
  pass "No npx @eai-tools/cli in deployment.mdx (nothing to check)"
fi
