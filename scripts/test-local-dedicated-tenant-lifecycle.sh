...
(async () => {
  const profile = await import('/Users/example/Desktop/EAI/Code/cloud-stack/eai/dist/lib/profile.js');
  profile.setActiveProfile(profileName);
  const { getAccessToken } = await import('/Users/example/Desktop/EAI/Code/cloud-stack/eai/dist/lib/auth.js');
